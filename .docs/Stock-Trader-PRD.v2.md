...
| v2.38 | 2026-06-21 | 환경변수 표준화 + Dockerfile + prod 기동 수정: ① `.env.dev`/`.env.staging` 서비스 URL `${VAR}` → 하드코딩(Docker Compose `env_file` 미확장 버그 수정). ② `.env.local` `AUTH_JWT_SECRET` 하드코딩값 제거(`<change-me>` 플레이스홀더). ③ 전 env 파일 섹션 구조 통일. ④ `docker-compose.yml` BFF `environment:` 에 5개 서비스 URL 명시. ⑤ `proxy.service.ts` 네트워크 오류 → 503(기존 500). ⑥ Python 서비스 4종 Dockerfile `COPY pyproject.toml uv.lock ./` + `uv sync --frozen --no-dev` + `ENV VIRTUAL_ENV/PATH` + `CMD ["uvicorn", ...]` 직접 실행(기존 `uv run` 런타임 재동기화·`|| true` 제거). ⑦ `Makefile` prod 타겟 전체 `-f docker-compose.yml` 추가 — `docker-compose.override.yml` 자동 병합 차단(소스 볼륨 마운트·`--reload` 오버라이드가 prod 컨테이너에서 uvicorn 실패 유발). |
...
| v2.40 | 2026-06-21 | F6.2.14 대시보드 종목비교 체크박스 ticker header 통합: ① `TickerHeader.tsx` 신규 client component — 체크박스(현재 종목 비교 즉시 추가/삭제) + ticker코드 + 종목명 + KOSPI/KOSDAQ 뱃지. `COMPARE_EVENT` 구독으로 다른 탭/검색 드롭다운 체크 시 즉시 동기화. ② `page.tsx` 기존 인라인 ticker header div 제거 → `<TickerHeader>` 교체(서버→클라이언트 경계 분리). TS clean. |
| v2.39 | 2026-06-21 | F6.2.14 대시보드 종목비교 UI 수정: ① `page.tsx` `CompareBar` 위치 ticker header 아래→위로 이동 — 페르소나+비교칩이 종목명보다 위에 표시. ② `TopBar.tsx` 히스토리 드롭다운(빈 입력 시)에도 체크박스 추가 — 기존 자동완성 모드에만 있어 히스토리 종목은 비교추가 불가였던 버그 수정. ③ Ticker header 폰트 조정(22→20px, 20→16px, alignItems baseline→center). ④ `CompareBar` 라벨 `추가순` → `종목비교목록 배열(추가순)` + `marginLeft: auto` 우측 정렬. TS clean. |
| v2.38 | 2026-06-21 | F6.2.14 대시보드 종목비교 + 페르소나 수정: ① `lib/compare-stocks.ts` 신규 — sessionStorage 기반 비교종목 목록(로그아웃/재기동 시 초기화), 10색 팔레트 자동 배정, `COMPARE_EVENT` 커스텀 이벤트. ② `CompareBar.tsx` 신규 — 페르소나 뱃지 + 비교종목 칩(추가순) 표시, 가격 변동 fetch 후 텍스트 색상 적용(상승=적색 `#f85149`, 하락=파란 `#58a6ff`). ③ TopBar 검색 드롭다운 체크박스 추가 — 각 종목 결과에 체크박스, `stopPropagation`으로 선택 이벤트 분리, `COMPARE_EVENT` 리스너로 실시간 동기화. ④ 페르소나 기본값 `swing` → `scalper` 전면 수정(`page.tsx`·`Sidebar.tsx`·`TopBar.tsx`). ⑤ TopBar `const persona = 'swing'` 하드코딩 버그 수정 → `navigate()` 호출 시 쿠키 실시간 읽기(Sidebar 변경값 보존). ⑥ Sidebar 로그아웃 시 `clearCompareList()` 호출. TS clean. |
| v2.37 | 2026-06-21 | F9.5 멀티계좌 영속화 수정: ① `paper_db.rs` — `save_all_snapshots`·`load_all_snapshots` 추가(`data/paper_books.json` v2 포맷, `_v: 2` + `accounts` 맵). 기존 `paper_book.json`(v1) 자동 마이그레이션. ② `paper.rs` — `snapshot_all_books()`·`restore_all_books()` 추가(BOOKS HashMap 전체 스냅샷/복원). ③ `main.rs` — 시작/종료/`paper_execute`/`paper_set_cash` 전체 전환: 단일 default 계좌 → 전체 계좌 저장(재기동 후 멀티계좌 유지). TS clean. |
| v2.36 | 2026-06-21 | F6.2.11 CandleChart4 전면 재설계: ① 레이아웃 2×2 → 3+1(상단 3열: 금일5분봉·시간대별 평균수익률·요일별 평균수익률, 하단 전폭: 일봉). ② 일봉 기본 기간 30→90일, 기간 셀렉터(90·180·270·365일) 우측상단 추가. ③ TA 패턴 감지 10종 인라인 구현 — BUY: 더블바텀·역헤드앤솔더·상승폴래그·상승삼각형·쏘사나단 / SELL: 더블탑·헤드앤솔더·하락폴래그·하락삼각형·쏘성바람. 패턴 신호 SVG 삼각형+텍스트로 일봉 차트에 표시. `candleLayout`·`applyLivePrice`·`priceRange`·`scaleY`(`@/lib/candles`) 활용. TS clean. |
